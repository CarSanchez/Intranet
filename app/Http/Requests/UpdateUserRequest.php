...
class UpdateUserRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'date' => ['bail', 'required'],
            'email' => ['bail', Auth::user()->role == 'sa' ? 'required' : '', 'max:100', Rule::unique('users', 'email')->ignore(Auth::user()->id)],
            'ext' => ['bail', Auth::user()->role == 'sa' ? 'required' : '', 'min:4', Rule::unique('users', 'ext')->ignore(Auth::user()->id)],
            $this->username() => ['bail', Auth::user()->role == 'sa' ? 'required' : '', 'min:2', 'max:20', Rule::unique('users', 'user')->ignore(Auth::user()->id)],
            'department' => ['bail', Auth::user()->role == 'sa' ? 'required' : ''],
